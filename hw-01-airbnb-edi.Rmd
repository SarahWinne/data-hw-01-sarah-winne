...
Each row represents the data for a certain neighborhood in Edinburgh.
...
There are too many different neighborhoods to fit them all next to each other, wrapping fits them all best while keeping the data readable. 
...
````{r}
hoods <- c("New Town", "Old Town", "West End", "Stockbridge", "Bruntsfield")

edibnb %>%
filter(neighbourhood %in% hoods) %>%
...
filter(neighbourhood %in% hoods) %>% 
  group_by(neighbourhood) %>%
summarise( min = min(price, na.rm=T), 
                    median = median(price, na.rm=T), 
                   stand.dev = sd(price, na.rm=T), 
                   IQR = IQR(price, na.rm=T), 
                   max = max(price, na.rm=T))
```

All of the prices graphed have a right skewed distribution. 
...
In general there are a lot of positive ratings. There are areas ratings are more concentrated in the lower areas too, at 20, 40, 60, and 80, so they also tend to give relatively simple fraction ratings. The neighborhoods, like Leith or New Town, that have a larger number of ratings also have more negative ratings than the neighborhoods with smaller numbers, like Tollcross or Stockbridge. 
